...
class GeminiHelper:
    """Helper class for Gemini AI integration for financial queries"""
    
    def __init__(self, api_key: str = None):
        """Initialize the Gemini helper"""
        self.api_key = "AIzaSyAYDim_dzAWZXvYGqemulbRmjhMN7rGxBE" or os.getenv("GEMINI_API_KEY") 
        ...
        if not self.api_key:
            logger.warning("Gemini API key not found. Limited functionality will be available.")
    ...
    def extract_query_components(self, query_text: str) -> Dict[str, Any]:
        ...
        # If no API key available, just return the locally extracted components
        if not self.api_key:
            logger.warning("No Gemini API key available. Using only local extraction.")
            return components
        ...
        try:
            ...
            if os.path.exists(cache_file):
                try:
                    ...
                    logger.info(f"Using cached Gemini results for: {query_text}")
                    return cached_data
                except Exception as e:
