...
class QueryProcessor:
    def __init__(self, news_collector, market_analyzer, gemini_helper=None):
        ...
        # Indian tickers and indices
        self.indian_tickers = {
            'RELIANCE.NS', 'TCS.NS', 'HDFCBANK.NS', 'INFY.NS', 'HINDUNILVR.NS',
            'ICICIBANK.NS', 'BHARTIARTL.NS', 'KOTAKBANK.NS', 'ITC.NS', 'SBIN.NS',
            'JYOTHYLAB.NS', 'NIFTY50', 'SENSEX'
        }
        
        # Combine ticker lists
        self.all_tickers = self.common_tickers.union(self.indian_tickers)
    ...
    def _extract_query_components(self, query_text):
        ...
        pattern1 = re.search(r'(?:why|what|how) (?:is|are|did|does) ([A-Za-z0-9]{1,5}) (?:up|down|going|moving|doing|performing)', query_text, re.IGNORECASE)
        if pattern1:
        ...
        pattern2 = re.search(r'(?:explain|about|analyze|check) ([A-Za-z0-9]{1,5})(?:\s|$)', query_text, re.IGNORECASE)
        if pattern2:
        ...
        pattern3 = re.search(r'(?:what|explain) (?:happened|occurring|going on|news) (?:to|with|for|about) ([A-Za-z0-9]{1,5})', query_text, re.IGNORECASE)
        if pattern3:
        ...
        # Special case for Indian stocks and indices (NIFTY, SENSEX, etc.)
        if 'nifty' in query_text:
            candidate_tickers.append('NIFTY50')
        if 'sensex' in query_text:
            candidate_tickers.append('SENSEX')
        if 'jyothy' in query_text or 'jyothy labs' in query_text:
            candidate_tickers.append('JYOTHYLAB.NS')
        ...
        # If we found known tickers, use those
    ...
    def _generate_answer(self, query_info, security_data, news_analysis, ticker):
        ...
        # Check if there are errors in security data
        if security_data and 'error' in security_data:
            return f"I couldn't retrieve data for {ticker}: {security_data['error']}"
    ...
    def _generate_price_movement_answer(self, security_data, news_analysis, ticker, query_info):
        ...
        try:
            ...
            if 'market_context' in security_data:
                ...
                if market_trends:
                    market_context = f" The broader market shows: {'; '.join(market_trends[:2])}."
    ...
    def _generate_performance_explanation(self, security_data, ticker, timeframe):
        ...
        try:
            ...
            if 'Volume' in data.columns:
                ...
                volume_change = ((data['Volume'].iloc[-1] - avg_volume) / avg_volume) * 100 if avg_volume > 0 else 0
            else:
                avg_volume = None
                max_volume = None
                volume_change = None
